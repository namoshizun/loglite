...
template<class Body>
inline http::response<http::string_body>
ok(const nlohmann::json& body, const http::request<Body>& req, std::string_view origin = "*") {
    return make_json_response(http::status::ok, body, req, origin);
}

template<class Body>
inline http::response<http::string_body>
fail(int status_code, std::string_view msg, const http::request<Body>& req,
     std::string_view origin = "*")
{
    return make_json_response(
        static_cast<http::status>(status_code),
        {{"error", msg}}, req, origin);
}
...
inline std::unordered_multimap<std::string, std::string>
...
inline std::vector<QueryFilter>
parse_filter_expr(std::string_view field, std::string_view expr) {
