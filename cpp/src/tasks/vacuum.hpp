...
inline int remove_stale_logs(Database& db, const Config& cfg) {
    ...
    if (n > 0)
        log::info(std::format("[vacuum] removed {} stale log(s) older than {} days", n, cfg.vacuum_max_days));
    return n;
}
...
inline int remove_excessive_logs(Database& db, const Config& cfg) {
    ...
    log::info(std::format(
        "[vacuum] db={:.1f}MB limit={:.1f}MB target={:.1f}MB – deleting id {} to {}",
...
inline int incremental_vacuum_pass(Database& db, int max_size_mb) {
    ...
    log::info(std::format("[vacuum] incremental_vacuum({}) pages in {:.1f}s", pages, t.elapsed_s()));
...
inline asio::awaitable<void> vacuum_task(ServerContext& ctx) {
    ...
    while (true) {
        ...
        if (has_ts)
            detail::remove_stale_logs(ctx.db, cfg);
