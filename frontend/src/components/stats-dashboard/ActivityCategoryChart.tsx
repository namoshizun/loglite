...
export default function ActivityCategoryChart({
  ...
  const { chartData, options } = useMemo(() => {
    ...
    const labels = data.map((row) => row.until);
      ...
      case 'query': {
        ...
        if (q.count) {
          datasets.push({
            ...
            data: data.map((row) => row.query_count),
        ...
        if (q.latency) {
          ...
          datasets.push({
            ...
            data: data.map((row) => [row.query_min, row.query_max] as [number, number]),
          ...
          datasets.push({
            ...
            data: data.map((row) => row.query_avg),
            borderColor: '#f59e0b',
      ...
      case 'ingestion': {
        ...
        if (ing.count) {
          datasets.push({
            ...
            data: data.map((row) => row.ingest_count),
        ...
        if (ing.size) {
          datasets.push({
            ...
            data: data.map((row) => row.ingest_size_avg),
            borderColor: '#60a5fa',
      ...
      case 'insertion': {
        ...
        if (ins.count) {
          datasets.push({
            ...
            data: data.map((row) => row.insert_total_count),
        ...
        if (ins.cost) {
          datasets.push({
            ...
            data: data.map((row) => row.insert_total_cost),
            borderColor: '#2dd4bf',
      ...
      case 'connections': {
        ...
        if (conn.http) {
          datasets.push({
            ...
            data: data.map((row) => row.http_conn_count),
            borderColor: '#ec4899',
        ...
        if (conn.sse) {
          datasets.push({
            ...
            data: data.map((row) => row.sse_session_count),
            borderColor: '#8b5cf6',
  ...
  }, [category, subMetrics, data, t, theme]);
