...
async def test_file_harvester_rotation(tmp_path, backlog):
    ...
    harvester = FileHarvester("rotation_test", FileHarvesterConfig(path=str(log_file)))
...
async def test_file_harvester_truncation(tmp_path, backlog):
    log_file = tmp_path / "test_trunc.log"
    log_file.touch()

    harvester = FileHarvester("truncation_test", FileHarvesterConfig(path=str(log_file)))
    await harvester.start()
    await asyncio.sleep(0.1)
